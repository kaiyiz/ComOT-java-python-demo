...
public class Mapping {
	...
	public Mapping(int n, double[] supplies, double[] demands, double[][] C, double delta) {
		...
		this.defLog = new ArrayList<double[]>();
		
		//Convert scaled flow 
		ArrayList<int[]> tempLog = gt.getDeficiencyLog();

		for (int[] tempScaledDef : tempLog) {
			double[] tempdef = new double[n];
			for (int i = 0; i < n; i++) tempdef[i] = (double)tempScaledDef[i] / (n * alpha); 
			defLog.add(tempdef);
		}
	...
	private ArrayList<double[]> defLog;
	...
	public double[] getDefLog(int idx){
		return defLog.get(idx);
	}
