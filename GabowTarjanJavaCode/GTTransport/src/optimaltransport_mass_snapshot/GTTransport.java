...
public class GTTransport {
	...
	private	ArrayList<int[]> deficiencyLog = new ArrayList<int[]>();
	...
	public ArrayList<int[]> getDeficiencyLog(){
		return deficiencyLog;
	}
	...
	public GTTransport(int[][] C, int[] supplies, int[] demands, int n) {
		...
		logRatio = new int[]{25, 50, 75, 85, 95, 98};
		int logCount = 0;
			...
			for (int vertex = 0; vertex < n; vertex++) {
				if (bFree[vertex]) {
					...
					while (deficiencyB[vertex] > 0 && vertexVisited[vertex] < 2*n - 1) {
						...
						curTransposed += beta;
						while(true){
							if(logCount < logPoint.length && logPoint[logCount] <= curTransposed){
								stopStep[logCount] = APinfo.size();
								deficiencyLog.add(deficiencyA.clone());
								deficiencyLog.add(deficiencyB.clone());
								logCount++;
								}
							else
								break;
